...
const ImageReader: React.FC<ImageReaderProps> = ({ images, title, onClose }) => {
    ...
    const hasPageB = twoPage && pageIndex + 1 < images.length;
    const pageBIndex = hasPageB ? pageIndex + 1 : -1;

    const progress = images.length > 1
        ? (pageIndex / (images.length - 1)) * 100
        : 100;

    const isFirst = pageIndex === 0;
    const isLast = twoPage && hasPageB
        ? pageIndex + 1 >= images.length - 1
    ...
    const goNext = () => {
        ...
        animKey.current += 1;
        const step = twoPage && hasPageB ? 2 : 1;
        goTo(pageIndex + step);
    };
    ...
    // Preload next pages
    const preloadIdxs = Array.from({ length: 3 }, (_, i) => pageIndex + 1 + i)
    ...
    const leftPage = rtl && twoPage ? pageBIndex : pageIndex;
    const rightPage = rtl && twoPage ? pageIndex : pageBIndex;
        ...
        <div style={{
            ...
            {!immersive && <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0, flex: 1, overflow: "hidden" }}>
                    ...
                    <PageInput
                        currentRealIndex={pageIndex}
                        total={images.length}
                        hasPageB={hasPageB}
                        pageBReal={pageBIndex}
                        goToReal={goTo}
                    />
                    <span style={{
            ...
            >
                ...
                >
                    {twoPage && hasPageB ? (
                        <>{renderRealPage(leftPage, "left")}{renderRealPage(rightPage, "right")}</>
                    ) : (
                        renderRealPage(pageIndex, "left")
                    )}
                </div>
            ...
            {!immersive && <div style={{ flexShrink: 0 }}>
                ...
                <div style={{
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                        {Array.from({ length: Math.min(7, images.length) }, (_, i) => {
                            const idx = Math.max(0, pageIndex - 3) + i;
                            if (idx >= images.length) return null;
                            return (
                                <button key={idx} onClick={() => goTo(idx)} style={{
                                    width: idx === pageIndex ? 24 : 8, height: 8, borderRadius: 4,
                                    background: idx === pageIndex ? "#e05c2a" : idx < pageIndex ? "#3a3a3a" : "#222",
